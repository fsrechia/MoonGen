...
function master(...)
    local arg = {...}
    local string_args = {}
    for index, param in pairs(arg) do
        print(index..": "..tostring(param))
        string_args[index] = tostring(param)
    end
    ...
    parser:option("--bths", "<back-to-back frame threshold>")

    parser:option("--duration", "<single test duration>")
    parser:option("--iterations", "<amount of test iterations>")

    parser:option("--din", "<DuT in interface name>")
    parser:option("--dout", "<DuT out iterface name>")
    parser:option("--dskip", "<skip DuT configuration>")

    parser:option("--asksshpass", "<true|false> [ask at beginning for SSH password]")
    parser:option("--sshpass", "<SSH password>")
    parser:option("--sshuser", "<SSH user>")
    parser:option("--sshport", "<SSH port>")
    parser:option("--asksnmpcomm", "<true|false> [ask at beginning for SNMP community string]")
    parser:option("--snmpcomm", "<SNMP community string>")
    parser:option("--host", "<mgmt host name of the DuT>")

    local arguments = parser:parse(string_args)
